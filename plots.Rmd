...
Not all runs went for the same amount of time, and some were refueled, which makes it harder to compare yields. So here are yields up to the 12-hour point (before any refueling took place):
```{r}
...
I was curious is selecting for high quality reads fixedbad demuxing, so I tried creating high quality subsets based on `barcode_score`, `mean_qscore_template` and `mean_identity`. However, none of these reliably resulted in a significant improvement.
...
This is the paper's main-text figure, comparing Illumina and ONT read depth for plasmids.
...
ont_vs_illumina_depth <- function(replicon_data, column_name, plot_title, y_title, point_colour, line_colour, line_fill_colour, regression) {
  ...
  # replicon_data = filter(replicon_data, Size < 20000)
...
ligation_to_illumina <- lm(ligation_log_depth ~ illumina_log_depth, data=replicon_data_no_chromosome)
...
identity_vs_translocation_speed <- function(read_table, plot_title, point_colour) {
  ...
  p <- ggplot(read_table) +
    ...
    labs(title = plot_title, x = "translocation speed", y = "identity")
...
identity_vs_start_time <- function(read_table, plot_title, point_colour) {
  ...
  p <- ggplot(read_table) +
    rasterise(geom_point(aes(x=start_time_hours, y=mean_identity), colour=point_colour, alpha=0.15, shape=16, size=0.5), dpi=600) +
    theme_bw() + theme(plot.background = element_blank()) +
    scale_y_continuous(breaks = seq(0.9, 1, 0.02),
                       labels = paste(seq(90, 100, 2), "%", sep = "")) +
    scale_x_continuous(breaks = seq(0, max_time, 6),
                       expand = c(0, 0)) +
    coord_cartesian(xlim = c(0, max_time), ylim = c(0.9, 1)) +
...
translocation_speed_vs_start_time <- function(read_table, plot_title, point_colour) {
  ...
  p <- ggplot(read_table) +
    ...
    labs(title = plot_title, x = "read time (hours)", y = "translocation speed")
